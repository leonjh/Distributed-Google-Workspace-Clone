...
void debug_new_conn(int fd) { std::cerr << "[" << fd << "] New connection" << std::endl; }

void debug_received(int fd, const std::string &msg) { std::cerr << "[" << fd << "] C: " << msg; }

void debug_sent(int fd, const std::string &msg) { std::cerr << "[" << fd << "] S: " << msg; }

void debug_closed_conn(int fd) { std::cerr << "[" << fd << "] S: Connection closed" << std::endl; }
...
void parse_args(int argc, char *argv[]) {
    ...
    if (std::string(argv[0]) == "./smtp" || std::string(argv[0]) == "./pop3" ||
        std::string(argv[0]) == "./send_mqueue") {
