...
{
private:
  const std::string greeting_message = "220 Service ready (Author: T07 Team)\r\n";
  ...
  const std::string shutting_down_message = "221 localhost Service closing transmission channel\r\n";
...
public:
   ...
   */
  SMTPServer () : threads(max_num_fds, -1), webmail_server(WebMailService{})
  {
    // int index = 0;
  ...
  void *worker_func(void *input)
  {
    ...
    std::vector<std::string> processed; // holds all full commands in buffer (last command may be unfinished)
    char buffer[1024];
    int num_bytes;
...
private:
  ...
  {
    json request_body;
    request_body["sender"] = state->sender;
    request_body["recipients"] = {};
    for (auto &recipient : state->recipients)
    {
      request_body["recipients"].push_back(recipient);
    }
    request_body["subject"] = "No Subject";
    request_body["body"] = state->email;

    webmail_server.send_email(request_body);
  }
  ...
  bool process(int connfd, const std::string &cmd, struct session_state *state)
  {
    ...
    else
    {
      ...
      else if (stripped.substr(0, 4) == "HELO")
      {
        ...
        if (state->sender.size() == 0 && state->recipients.size() == 0 && state->email.size() == 0)
        {
      ...
      {
        if (!state->received_hello || state->sender.size() == 0 || state->recipients.size() == 0)
        {
          write_response(connfd, bad_sequence_message);
        }
        else
...
void int_handler_wrapper(int sig)
{
  smtp_server->int_handler(sig);
}
